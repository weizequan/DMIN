...
from .models_a2v import New_Audio_Guided_Attention
from .models_a2v import EncoderLayer, Encoder, DecoderLayer, Decoder
...
class AudioVideoInter(nn.Module):
    ...
    def forward(self, video_feat, audio_feat,labels):
        t_size,batch,channel=audio_feat.shape
        audio_mean = []
        ...
        for i in range(batch):
            ...
            for j in range(t_size):        
                if(labels[i][j]==1):
                    ...
                    temp = temp.double()
                    temp =audio_feat[j,i,:]
                    audio_sum.append(temp.unsqueeze(0))
            audio_sum = torch.cat(audio_sum,0)
            audio_mean = audio_sum.mean(dim=-1)
            t = audio_mean.size(0)
            audio_mean = audio_mean.unsqueeze(-1)
            k=0
            while k+t<=t_size:               
                video_feat[k:k+t,i,:]*= audio_mean
                k=k+1
