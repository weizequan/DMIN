...
from model.weakly_main_model import weak_main_model as main_model
...
def train_epoch(model, train_dataloader, criterion, criterion_event, optimizer, epoch):
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        labels = labels.cuda()
        is_event_scores, raw_logits, event_scores,event_self = model(visual_feature, audio_feature)
        ...
        loss_event_self =criterion(event_self,labels.double())
        # loss = loss_is_event + loss_event_class
        loss = 0.92*loss_event_class+0.08*loss_event_self
